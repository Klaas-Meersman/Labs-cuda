...
info matrix_multiplication_run(int rowsA, int colsA, int rowsB, int colsB, dim3 numBlocks, dim3 threadsPerBlock)
{
...
info matrix_multiplication_run_shared(int rowsA, int colsA, int rowsB, int colsB, dim3 numBlocks, dim3 threadsPerBlock)
{
    // Define small matrices A (FxG), B (GxH), and C (FxH)
    if (colsA != rowsB)
    {
...
info matrix_multiplication_run_const(int rowsA, int colsA, int rowsB, int colsB, dim3 numBlocks, dim3 threadsPerBlock)
{
    // Define small matrices A (FxG), B (GxH), and C (FxH)
...
int main(void)
{
    // change the gridSize, blockSize. increase gridsize every iteration, matrix stays the same size (1024x1024)

    const int const_size = 512;

    const int numberOfReruns = 10;

    // intialize all the gridsizes
    std::vector<dim3> gridSizes;
    ...
    {
        gridSizes.push_back(dim3(i, i, 1));
        blockSizes.push_back(dim3(blockSizeX, blockSizeY, 1));
    }
    ...
    for (int i = 0; i < gridSizes.size(); i++)
    {
        ...
        {
            dim3 blockSize = blockSizes[i];
            dim3 gridSize = gridSizes[i];

            info info_global = matrix_multiplication_run(const_size, const_size, const_size, const_size, blockSize, gridSize);
            info info_shared = matrix_multiplication_run_shared(const_size, const_size, const_size, const_size, blockSize, gridSize);
            info info_const = matrix_multiplication_run_const(const_size, const_size, const_size, const_size, blockSize, gridSize);
